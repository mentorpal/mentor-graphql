...
describe('buildMentor', () => {
  ...
  it(`throws an error if mentor is not the user's`, async () => {
    ...
    const response = await request(app)
      ...
      .send({
        query: `mutation {
          me {
            buildMentor(mentorId: "5ffdf41a1ee2c62320b49ea1") {
              _id
            }
          }
        }`,
  ...
  it('builds mentor', async () => {
    ...
    expect(response.status).to.equal(200);
    expect(response.body.data.me.buildMentor).to.eql({
